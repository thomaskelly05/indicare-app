...
const fallbackWorkspace: WorkspaceData = {
  ...
  reviews: [
    ...
    },
    {
      ...
      summary: "Review required because three recent records mention anxiety before family contact.",
      status: "Due today",
      priority: "high",
      date: "Today 21:00",
  ...
  plans: [
    {
      ...
      summary: "Use predictable preparation before contact. Jamie should be told arrangements early and offered quiet time afterwards.",
    ...
    },
    {
      ...
      summary: "Current risk level medium. No recent missing episode, but staff should monitor transition points and contact-related anxiety.",
      status: "Current",
      priority: "medium",
  ...
  alerts: [
    {
      ...
      summary: "Recent contact planning note does not clearly evidence whether advocacy was offered, accepted or declined.",
      status: "Open",
      priority: "medium",
      date: "Today",
  ...
  documents: [
    {
      ...
      summary: "Care plan summary uploaded and linked to placement plan, risk plan and latest LAC review.",
      status: "Current",
      priority: "normal",
  ...
  handover: [
    {
      ...
      summary: "Monitor mood before tomorrow's contact plan. Jamie settled after quiet activity and reassurance.",
      status: "Draft ready",
      priority: "medium",
      date: "Tonight",
  ...
  childVoice: [
    {
      ...
      summary: "Jamie said: 'I just want to know what is happening before people start talking about it.'",
...
function priorityClass(priority?: WorkspaceRecord["priority"]) {
...
function normaliseApiWorkspace(raw: any, childId: string): WorkspaceData {
  ...
  const toRecord = (item: any, index: number, typeFallback: string): WorkspaceRecord => ({
    ...
    priority: String(item?.priority || item?.severity || "normal").toLowerCase() as WorkspaceRecord["priority"],
...
export default function YoungPersonWorkspacePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    const base = process.env.NEXT_PUBLIC_INDICARE_API_BASE || "";
    fetch(`${base}/api/os-command/young-person/${params.id}/workspace`, {
      credentials: "include",
      signal: controller.signal,
    })
      .then((response) => (response.ok ? response.json() : null))
      .then((data) => {
        if (data) setWorkspace(normaliseApiWorkspace(data, params.id));
      })
      .catch(() => undefined);
  ...
  function updateSelected(field: keyof WorkspaceRecord, value: string) {
    ...
    setSelected(updated);
    const replace = (items: WorkspaceRecord[]) => items.map((item) => (item.id === updated.id ? updated : item));
    setWorkspace((current) => ({
      ...current,
      records: replace(current.records),
      reviews: replace(current.reviews),
      plans: replace(current.plans),
      alerts: replace(current.alerts),
      documents: replace(current.documents),
      lifeEcho: replace(current.lifeEcho),
      handover: replace(current.handover),
      childVoice: replace(current.childVoice),
      appointments: replace(current.appointments),
    }));
  }

  function runOrb(question: string) {
    setOrbMode(question);
    const lower = question.toLowerCase();
    const lac = workspace.reviews.find((item) => item.type.toLowerCase().includes("lac"));
    const dentist = workspace.appointments.find((item) => item.type.toLowerCase().includes("dentist") || item.title.toLowerCase().includes("dentist"));
    const highAlerts = workspace.alerts.filter((item) => item.priority === "high" || item.priority === "critical");

    if (lower.includes("lac")) {
      setOrbAnswer(lac ? `${lac.title}: ${lac.summary} Evidence: ${lac.evidence}` : "I cannot find a recorded LAC review in this workspace. Check Documents and Reviews, then add an evidence gap if it is missing.");
      return;
    }
    if (lower.includes("dentist")) {
      setOrbAnswer(dentist ? `${dentist.title}: ${dentist.date}. ${dentist.summary} Next action: ${dentist.action}` : "I cannot find a dentist appointment in this workspace. Check Health documents and appointment records.");
      return;
    }
    if (lower.includes("therapeutic")) {
      setOrbAnswer("Therapeutic rewrite: Jamie became distressed after the phone call. Staff used calm reassurance, offered space and supported Jamie to settle. Record what Jamie said or showed, what staff did, whether anyone was harmed, and what follow-up is needed.");
      return;
    }
    if (lower.includes("ofsted")) {
      setOrbAnswer("Ofsted lens: show that staff understand Jamie's needs, risks are reviewed when patterns emerge, child voice influences planning, and manager oversight is recorded with clear rationale.");
      return;
    }
    if (lower.includes("review")) {
      setOrbAnswer(highAlerts.length ? `${highAlerts.length} high-priority item(s) need review. Start with: ${highAlerts[0].title}. ${highAlerts[0].action}` : "No high-priority manager review is showing in this workspace. Check due dates before sign-off.");
      return;
    }
    setOrbAnswer("I can help find evidence, explain patterns, draft records, check review dates, summarise the child journey and highlight what an inspector or manager may ask next.");
  }
    ...
    <main className="min-h-screen bg-[#eef4fb] text-slate-950">
      <div className="grid min-h-screen grid-cols-[260px_minmax(0,1fr)_336px] max-[1180px]:grid-cols-[230px_minmax(0,1fr)] max-[900px]:block">
          ...
          <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 px-6 py-4 backdrop-blur max-[900px]:relative">
            ...
            <div className="mt-4 flex gap-2 overflow-x-auto">
              {tabs.slice(0, 5).map((tab) => (
                <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`rounded-full px-5 py-2 text-xs font-black uppercase tracking-[0.18em] ${activeTab === tab ? "bg-blue-600 text-white" : "border border-slate-200 bg-white text-slate-500"}`}>
                  {tab}
                </button>
              ))}
            </div>
          ...
          <div className="p-6">
              ...
              <div className="flex items-start gap-6 max-[760px]:block">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
                    Known as <strong>{workspace.child.preferredName}</strong>. {workspace.child.communication} {workspace.child.whatHelps}
                  </p>
              ...
              <section className="mt-6 rounded-[1.6rem] bg-white p-6 shadow-sm">
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 max-[760px]:grid-cols-1">
                  {recordRoutes.map((route) => (
                    <button key={route} type="button" onClick={() => setSelected({ id: `new-${route}`, type: route, title: route, summary: "Start recording here. ORB will help you stay factual, therapeutic and evidence-led.", status: "Draft", priority: "normal", evidence: "", action: "", owner: "Current user" })} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left font-black transition hover:border-blue-300 hover:bg-blue-50">
                      {route}
                      <span className="mt-1 block text-xs font-semibold text-slate-500">Open editable route</span>
                    </button>
                  ))}
                </div>
              </section>
            ...
            <section className="mt-6 rounded-[1.6rem] bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <div className="text-[10px] font-black uppercase tracking-[0.35em] text-slate-400">{activeTab}</div>
                  <h3 className="mt-2 text-2xl font-black">Open, edit and evidence</h3>
                </div>
                ...
                {activeItems.length ? activeItems.map((item) => (
                  <button key={item.id} type="button" onClick={() => setSelected(item)} className={`w-full rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 hover:shadow-lg ${priorityClass(item.priority)}`}>
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <div className="text-[10px] font-black uppercase tracking-[0.25em] opacity-60">{item.type} · {item.status}</div>
                        <h4 className="mt-2 text-lg font-black">{item.title}</h4>
                        <p className="mt-2 text-sm leading-6 opacity-75">{item.summary}</p>
                      </div>
                      <span className="rounded-full bg-white/70 px-3 py-1 text-xs font-black">Open</span>
                    </div>
                  </button>
                )) : (
        ...
        <aside className="sticky top-0 h-screen overflow-y-auto border-l border-slate-200 bg-white/70 p-5 backdrop-blur max-[1180px]:col-span-2 max-[1180px]:h-auto max-[900px]:relative">
          <section className="rounded-[1.4rem] bg-slate-950 p-5 text-white shadow-xl">
            ...
            <div className="mt-5 space-y-2">
              {["When was the last LAC review?", "When was the last dentist appointment?", "Rewrite this therapeutically", "What needs manager review?", "What would Ofsted ask?"].map((question) => (
                <button key={question} type="button" onClick={() => runOrb(question)} className="w-full rounded-xl border border-blue-400/40 bg-white/5 px-3 py-3 text-left text-xs font-black text-blue-50 transition hover:bg-blue-500/20">
                  {question}
                </button>
              ))}
            </div>
          ...
          <section className="mt-5 rounded-[1.4rem] bg-white p-5 shadow-sm">
            ...
            {orbMode.toLowerCase().includes("therapeutic") && (
              <div className="mt-4">
                <label className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Rough wording</label>
                <textarea value={orbDraft} onChange={(event) => setOrbDraft(event.target.value)} className="mt-2 min-h-28 w-full rounded-2xl border border-slate-200 bg-white p-3 text-sm text-slate-800" />
              </div>
            )}
          </section>
          ...
          <section className="mt-5 rounded-[1.4rem] bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
              {workspace.alerts.concat(workspace.handover).slice(0, 4).map((item) => (
                <button key={`todo-${item.id}`} type="button" onClick={() => setSelected(item)} className="w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left transition hover:bg-blue-50">
      ...
      {selected && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4 backdrop-blur-sm">
          <section className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-[10px] font-black uppercase tracking-[0.35em] text-blue-600">Editable workspace item</div>
                <h3 className="mt-2 text-3xl font-black tracking-tight">{selected.type}</h3>
              </div>
              <button type="button" onClick={() => setSelected(null)} className="rounded-full bg-slate-100 px-4 py-2 text-sm font-black">Close</button>
            </div>
            <div className="mt-5 grid gap-4">
              <label className="grid gap-2 text-xs font-black uppercase tracking-[0.2em] text-slate-400">Title<input value={selected.title} onChange={(event) => updateSelected("title", event.target.value)} className="rounded-2xl border border-slate-200 p-4 text-base normal-case tracking-normal text-slate-900" /></label>
              <label className="grid gap-2 text-xs font-black uppercase tracking-[0.2em] text-slate-400">Summary<textarea value={selected.summary} onChange={(event) => updateSelected("summary", event.target.value)} className="min-h-28 rounded-2xl border border-slate-200 p-4 text-base normal-case tracking-normal text-slate-900" /></label>
              <label className="grid gap-2 text-xs font-black uppercase tracking-[0.2em] text-slate-400">Evidence<textarea value={selected.evidence || ""} onChange={(event) => updateSelected("evidence", event.target.value)} className="min-h-24 rounded-2xl border border-slate-200 p-4 text-base normal-case tracking-normal text-slate-900" /></label>
              <label className="grid gap-2 text-xs font-black uppercase tracking-[0.2em] text-slate-400">Action<textarea value={selected.action || ""} onChange={(event) => updateSelected("action", event.target.value)} className="min-h-24 rounded-2xl border border-slate-200 p-4 text-base normal-case tracking-normal text-slate-900" /></label>
              <div className="grid grid-cols-2 gap-4 max-[640px]:grid-cols-1">
                <label className="grid gap-2 text-xs font-black uppercase tracking-[0.2em] text-slate-400">Status<input value={selected.status} onChange={(event) => updateSelected("status", event.target.value)} className="rounded-2xl border border-slate-200 p-4 text-base normal-case tracking-normal text-slate-900" /></label>
                <label className="grid gap-2 text-xs font-black uppercase tracking-[0.2em] text-slate-400">Owner<input value={selected.owner || ""} onChange={(event) => updateSelected("owner", event.target.value)} className="rounded-2xl border border-slate-200 p-4 text-base normal-case tracking-normal text-slate-900" /></label>
              </div>
            </div>
            <div className="mt-6 flex flex-wrap gap-3">
              <button type="button" onClick={() => setSelected(null)} className="rounded-2xl bg-blue-600 px-5 py-3 text-sm font-black text-white">Save locally</button>
              <button type="button" onClick={() => runOrb("Rewrite this therapeutically")} className="rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-700">Ask ORB to improve wording</button>
              <button type="button" onClick={() => runOrb("What would Ofsted ask?")} className="rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-700">Ofsted lens</button>
            </div>
          </section>
        </div>
      )}
      ...
      {isEditingChild && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4 backdrop-blur-sm">
          <section className="w-full max-w-2xl rounded-[2rem] bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-[10px] font-black uppercase tracking-[0.35em] text-blue-600">Edit child understanding</div>
                <h3 className="mt-2 text-3xl font-black tracking-tight">{workspace.child.name}</h3>
              </div>
              <button type="button" onClick={() => setIsEditingChild(false)} className="rounded-full bg-slate-100 px-4 py-2 text-sm font-black">Close</button>
            </div>
            <div className="mt-5 grid gap-4">
              <label className="grid gap-2 text-xs font-black uppercase tracking-[0.2em] text-slate-400">Communication<textarea value={workspace.child.communication} onChange={(event) => setWorkspace((current) => ({ ...current, child: { ...current.child, communication: event.target.value } }))} className="min-h-24 rounded-2xl border border-slate-200 p-4 text-base normal-case tracking-normal text-slate-900" /></label>
              <label className="grid gap-2 text-xs font-black uppercase tracking-[0.2em] text-slate-400">What helps<textarea value={workspace.child.whatHelps} onChange={(event) => setWorkspace((current) => ({ ...current, child: { ...current.child, whatHelps: event.target.value } }))} className="min-h-24 rounded-2xl border border-slate-200 p-4 text-base normal-case tracking-normal text-slate-900" /></label>
            </div>
            <button type="button" onClick={() => setIsEditingChild(false)} className="mt-6 rounded-2xl bg-blue-600 px-5 py-3 text-sm font-black text-white">Save locally</button>
          </section>
        </div>
      )}
    </main>
