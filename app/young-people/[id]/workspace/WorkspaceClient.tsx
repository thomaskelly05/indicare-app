...
export function WorkspaceClient({ childId }: { childId: string }) {
  ...
  const activeItems = useMemo(() => {
    if (activeTab === "Overview") return [...workspace.commandItems, ...workspace.alerts, ...workspace.compliance, ...workspace.reviews, ...workspace.records].slice(0, 12);
    ...
    if (activeTab === "Calendar") return workspace.calendar;
    if (activeTab === "Documents") return workspace.documents;
    if (activeTab === "Compliance") return workspace.compliance;
    if (activeTab === "Standards") return workspace.standards;
    if (activeTab === "Reports") return workspace.reports;
  ...
  const actionRail = [...workspace.commandItems, ...workspace.alerts, ...workspace.compliance, ...workspace.handover].slice(0, 5);
    ...
    <main className="min-h-screen bg-[#eef4fb] text-slate-950">
      <div className="grid min-h-screen grid-cols-[260px_minmax(0,1fr)_336px] max-[1180px]:grid-cols-[230px_minmax(0,1fr)] max-[900px]:block">
        <aside className="sticky top-0 h-screen overflow-y-auto border-r border-slate-200 bg-[#030918] p-4 text-white max-[900px]:relative max-[900px]:h-auto">
          ...
          <nav className="mt-6 space-y-6"><section><div className="mb-2 px-2 text-[10px] font-black uppercase tracking-[0.35em] text-slate-500">Child - primary</div>{tabs.slice(0, 8).map((tab) => <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`mb-1 flex w-full items-center justify-between rounded-2xl px-4 py-3 text-left text-sm font-black transition ${activeTab === tab ? "bg-white text-slate-950" : "text-slate-300 hover:bg-white/10"}`}><span>{tab}</span><span className="text-xs opacity-60">›</span></button>)}</section><section><div className="mb-2 px-2 text-[10px] font-black uppercase tracking-[0.35em] text-slate-500">Child - more</div>{tabs.slice(8).map((tab) => <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`mb-1 flex w-full items-center justify-between rounded-2xl px-4 py-3 text-left text-sm font-black transition ${activeTab === tab ? "bg-white text-slate-950" : "text-slate-300 hover:bg-white/10"}`}><span>{tab}</span><span className="text-xs opacity-60">›</span></button>)}</section></nav>
        </aside>
