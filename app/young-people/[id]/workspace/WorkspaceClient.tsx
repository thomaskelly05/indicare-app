  ...
  records: [
    {
      ...
      summary: "Jamie accepted reassurance, completed the evening routine and remained settled.",
      status: "Submitted",
      priority: "normal",
      date: "Today",
      evidence: "Routine completed and child spoken with privately.",
      action: "Keep the plan visible during handover.",
      owner: "Night staff",
    },
  ],
  reviews: [],
...
export function WorkspaceClient({ childId }: { childId: string }) {
  ...
  async function saveSelected() {
    ...
    const result = await saveChildWorkspaceItem(childId, {
      ...
      payload: { tab: activeTab, date: selected.date, ...recordDetails },
    });
      ...
      <div className="grid min-h-screen grid-cols-[260px_minmax(0,1fr)_336px] max-[1180px]:grid-cols-[230px_minmax(0,1fr)] max-[900px]:block">
          ...
          <div className="p-6">
            ...
            {activeTab === "Record" && (
              <section className="mt-6 rounded-[1.6rem] bg-white p-6 shadow-sm">
                ...
                <p className="mt-2 text-sm text-slate-600">Every route now opens fields that match the canonical database table.</p>
                <div className="mt-5 grid grid-cols-2 gap-3 max-[760px]:grid-cols-1">
                  {recordRoutes.map((route) => (
                    <button key={route} type="button" onClick={() => newItem(route)} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left font-black transition hover:border-blue-300 hover:bg-blue-50">
                      {route}<span className="mt-1 block text-xs font-semibold text-slate-500">Open typed record form</span>
                    </button>
                  ))}
                </div>
              </section>
            ...
            <section className="mt-6 rounded-[1.6rem] bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between gap-4">
                ...
                <button type="button" onClick={() => newItem(activeTab)} className="rounded-2xl bg-slate-950 px-4 py-3 text-xs font-black text-white">+ Add</button>
              </div>
              <div className="mt-5 space-y-3">
                {activeItems.length ? activeItems.map((item) => (
                  <button key={item.id} type="button" onClick={() => openItem(item)} className={`w-full rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 hover:shadow-lg ${priorityClass(item.priority)}`}>
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <div className="text-[10px] font-black uppercase tracking-[0.25em] opacity-60">{item.type} · {item.status} {item.date ? `· ${item.date}` : ""}</div>
                        <h4 className="mt-2 text-lg font-black">{item.title}</h4>
                        <p className="mt-2 text-sm leading-6 opacity-75">{item.summary}</p>
                        <p className="mt-2 text-xs font-bold opacity-60">{item.action}</p>
                      </div>
        ...
        <aside className="sticky top-0 h-screen overflow-y-auto border-l border-slate-200 bg-white/70 p-5 backdrop-blur max-[1180px]:col-span-2 max-[1180px]:h-auto max-[900px]:relative">
          ...
          <section className="mt-5 rounded-[1.4rem] bg-white p-5 shadow-sm">
            ...
            {orbEvidence.length > 0 && <div className="mt-3 space-y-2">{orbEvidence.map((item) => <button key={item.id} type="button" onClick={() => openItem(item)} className="w-full rounded-2xl border border-slate-200 bg-white p-3 text-left text-xs text-slate-600"><strong className="block text-slate-900">{item.title}</strong>{item.summary}</button>)}</div>}
          ...
          <section className="mt-5 rounded-[1.4rem] bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3">{actionRail.map((item) => <button key={`todo-${item.id}`} type="button" onClick={() => openItem(item)} className="w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left transition hover:bg-blue-50"><div className="font-black">{item.title}</div><div className="mt-1 text-xs text-slate-500">{item.action}</div></button>)}</div>
          </section>
        </aside>
      </div>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4 backdrop-blur-sm">
          <section className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              ...
              <button type="button" onClick={() => { setSelected(null); setRecordDetails({}); }} className="rounded-full bg-slate-100 px-4 py-2 text-sm font-black">Close</button>
            </div>
            <div className="mt-5 grid gap-4">
              ...
              <RecordExtraFields type={selected.type} values={recordDetails} onChange={(name, value) => { setRecordDetails((current) => ({ ...current, [name]: value })); setSaveState("Unsaved changes."); }} />
              <label className="grid gap-2 text-xs font-black uppercase tracking-[0.2em] text-slate-400">Evidence / purpose<textarea value={selected.evidence || ""} onChange={(event) => updateSelected("evidence", event.target.value)} className="min-h-24 rounded-2xl border border-slate-200 p-4 text-base normal-case tracking-normal text-slate-900" /></label>
              <label className="grid gap-2 text-xs font-black uppercase tracking-[0.2em] text-slate-400">Action / follow-up<textarea value={selected.action || ""} onChange={(event) => updateSelected("action", event.target.value)} className="min-h-24 rounded-2xl border border-slate-200 p-4 text-base normal-case tracking-normal text-slate-900" /></label>
              ...
              <ManagerActionButtons childId={childId} item={selected} onStatus={setSaveState} onComplete={async () => { await loadWorkspace("Refreshing workspace after manager action..."); }} />
            </div>
            <div className="mt-6 flex flex-wrap gap-3">
              <button type="button" onClick={saveSelected} className="rounded-2xl bg-blue-600 px-5 py-3 text-sm font-black text-white">Save to workspace</button>
